...
def trainIters(encoder, decoder, n_iters,epochs=5, print_every=100, plot_every=100, learning_rate=0.01,n_evaluate=1000):
    ...
    for epoch in range(0,epochs):
        ...
        loss_PATH="lstmlosses_epoch"+str(epoch)
        loss_test_PATH="lstmlosses_test_epoch"+str(epoch)
        np.save(loss_PATH,np.array(all_losses))
        np.save(loss_test_PATH,np.array(all_test_losses))

        enco_PATH="lstmencoder_epoch"+str(epoch)
        deco_PATH="lstmdecoder_epoch"+str(epoch)
        torch.save(encoder,enco_PATH)
        torch.save(encoder,deco_PATH)

    torch.save(encoder,"lstmsecond_enco")
    torch.save(encoder,"lstmsecond_deco")       
    np.save('lstmall_losses',np.array(all_losses))
    np.save('lstmall_test_losses',np.array(all_losses))
...
trainIters(encoder1, attn_decoder1, n_iters=20000,epochs=1, print_every=100,plot_every=10)
